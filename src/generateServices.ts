...
function renderMethod(op: any) {
  ...
  // Use normalized param names to match the Next.js [id] convention
  const normalizedParams: string[] = pathParams.map((p: string) => normalizeParamName(p, pathParams));

  const nextPath = toNextPath(path);
  const urlExpr = normalizedParams.reduce(
    (u: string, p: string) => u.replace(`[${p}]`, `\${${p}}`),
    nextPath,
  );

  const urlStr = normalizedParams.length > 0 ? `\`/api${urlExpr}\`` : `'/api${urlExpr}'`;
