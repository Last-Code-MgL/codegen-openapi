...
function renderHandler(op: any, { apiEnvVar, apiFallback }: any) {
  ...
  const isJson        = hasBody && !isMultipart; // trata todo o resto como JSON
  ...
  // Body — diferencia JSON de multipart
  ...
  if (isJson) {
    ...
    // Para multipart: passamos o FormData diretamente.
    // NÃO definimos Content-Type — o fetch/axios precisa gerar o boundary automaticamente.
    lines.push(`    const formData = await request.formData();`);
    fetchOpts.push('body: formData');
    lines.push(`    // multipart/form-data — boundary gerado automaticamente, não forçar Content-Type`);
  }

  lines.push(`    const response = await fetchBackend(url, { ${fetchOpts.join(', ')} });`);
  lines.push('');

  // Response handling (agnóstico ao content-type de resposta)
  lines.push(`    let data;`);
  ...
  lines.push(`        { success: false, message: data?.message ?? 'Erro na requisição' },`);
  ...
  lines.push(`    return NextResponse.json({ success: false, message: 'Erro interno do servidor' }, { status: 500 });`);
  lines.push(`  }`);
  lines.push(`}`);
...
export async function generateRoutes({
  ...
  // Agrupa por path Next.js (vários métodos → mesmo route.ts)
